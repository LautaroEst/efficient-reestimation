...
def calibrate_and_evaluate_psr(scores, targets, metric="LogLoss", calmethod='AffineCalLogLoss', use_bias=True, deploy_priors=None, cross_val=False, bootstrap=False, random_state=0):
    ...
    if not cross_val:
        ...
        scores_tst_cal = calibration_with_crossval(scores, targets, use_bias=True, priors=deploy_priors, calmethod=calmethod)
        _, calmodel = calibration_train_on_heldout(scores, scores, targets, use_bias=True, priors=deploy_priors, calmethod=calmethod)
