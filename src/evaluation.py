...
def get_content_free_input_probs(model, dataset, content_free_inputs, batch_size=1, prompt_func=None):
    all_probs = []
    for prompt in dataset.random_batch_loader_from_list(content_free_inputs, num_samples=None, batch_size=batch_size, prompt_func=prompt_func):
        probs = model.get_label_probs(prompt, dataset.label_dict)
        all_probs.append(probs)
    all_probs = np.vstack(all_probs) ## ACÁ HAY ALGO RARO. NO DAN LAS MISMAS PROBS QUE EN EL CÓDIGO ORIGINAL
    mean_probs = all_probs.mean(axis=0)
    mean_probs_norm = mean_probs / mean_probs.sum() # Normalize
    return mean_probs_norm


def get_train_queries_probs(model, dataset, num_train_samples=100, batch_size=32, prompt_func=None):
    _, all_probs, _, _, _ = _get_probs_from_split(model, dataset, eval_split="train", num_samples=num_train_samples, batch_size=batch_size, prompt_func=prompt_func)
    mean_probs = all_probs.mean(axis=0)
    mean_probs_norm = mean_probs / mean_probs.sum() # Normalize
    return mean_probs_norm
